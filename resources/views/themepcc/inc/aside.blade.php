                        ...
                        <ul class="list-group list-group-movie clearfix">
                            @foreach ($top['data'] as $movie)
                                <li class="list-group-item clearfix">
                                    <div class="thumbnail">
                                        <a href="{{ $movie->episodes->sortBy([['server', 'asc']])->groupBy('server')->first()->sortByDesc('name', SORT_NATURAL)->groupBy('name')->last()->sortByDesc('type')->first()->getUrl() }}" title="{{ $movie->name }}">
                                            <img data-src="{{ $movie->getThumbUrl() }}" class="lazyload" alt="{{ $movie->name }}" />
                                        </a>
                                    </div>
                                    <div class="meta-item">
                                        <h3 class="name-1">
                                            <a href="{{ $movie->episodes->sortBy([['server', 'asc']])->groupBy('server')->first()->sortByDesc('name', SORT_NATURAL)->groupBy('name')->last()->sortByDesc('type')->first()->getUrl() }}"
