            ...
            <div class="block">
                <div class="block-heading with-tabs">
                    <div class="block-title">
                        <h2 class="block-title"><i class="sp-movie-icon-cup"></i> Phim Sex Đề Cử</h2>
                    </div>
                </div>
                    ...
                    <div id="owl-slide" class="row-fluid">
                        <div id="phim-hay" class="block-items row fix-row">
                            @foreach ($recommendations as $movie)
                                <div class="item">
                                    <a class="inner" href="{{ $movie->episodes->sortBy([['server', 'asc']])->groupBy('server')->first()->sortByDesc('name', SORT_NATURAL)->groupBy('name')->last()->sortByDesc('type')->first()->getUrl() }}" title="{{$movie->name}}">
